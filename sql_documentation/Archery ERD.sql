...
CREATE TYPE "role_enum" AS ENUM (
  'admin',
  'australia_archery_federation',
  'recorder',
  'archer'
);

CREATE TYPE "social_connection_request_enum" AS ENUM (
  'pending',
  'accepted',
  'rejected'
);
...
CREATE TABLE "account" (
  ...
  "avatar_url" text DEFAULT 'https://ghcpcyvethwdzzgyymfp.supabase.co/storage/v1/object/public/User%20Uploaded/Account_Avatar/Default_Account_Avatar.jpg',
...
CREATE TABLE "club" (
  ...
  "club_logo_url" text NOT NULL DEFAULT 'https://ghcpcyvethwdzzgyymfp.supabase.co/storage/v1/object/public/User%20Uploaded/Club_Logo/Default_Club_Logo.png',
  "created_at" timestamptz NOT NULL,
  "updated_at" timestamptz NOT NULL
);
...
CREATE TABLE "participating" (
  ...
  "score_5th_arrow" smallint NOT NULL DEFAULT 0,
  "score_6th_arrow" smallint NOT NULL DEFAULT 0,
...
CREATE TABLE "event_context" (
  "event_context_id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "yearly_club_championship_id" int,
  "club_competition_id" int NOT NULL,
...
CREATE TABLE "yearly_club_championship" (
  "yearly_club_championship_id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "name" varchar(255) UNIQUE NOT NULL,
  "year" int NOT NULL,
  "creator_id" int NOT NULL,
...
CREATE TABLE "club_competition" (
  "club_competition_id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "name" varchar(255) UNIQUE NOT NULL,
...
CREATE TABLE "round" (
  "round_id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "name" varchar(255) UNIQUE NOT NULL,
...
CREATE TABLE "equipment" (
  ...
  "photo_url" text NOT NULL DEFAULT 'https://ghcpcyvethwdzzgyymfp.supabase.co/storage/v1/object/public/User%20Uploaded/Equipment_Photo/Default_Equipment_Photo.png',
...
CREATE TABLE "eligible_group_of_club" (
  "eligible_group_of_club_id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "created_at" timestamptz NOT NULL
);
...
CREATE TABLE "age_division" (
  "age_division_id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE "discipline" (
  "discipline_id" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "name" varchar(255),
...
CREATE TABLE "category_rating_percentile" (
  "archer_id" int,
  "category_id" int,
  "percentile" smallint NOT NULL DEFAULT 0,
...
CREATE TABLE "friendship_request_form" (
  ...
  "status" social_connection_request_enum NOT NULL DEFAULT 'pending',
  "created_at" timestamptz NOT NULL,
  "updated_at" timestamptz NOT NULL,
...
CREATE INDEX "year-eligible_group_of_club_id" ON "yearly_club_championship" ("year", "eligible_group_of_club_id");

CREATE INDEX "date_start-eligible_group_of_club_id" ON "club_competition" ("date_start", "eligible_group_of_club_id");
...
COMMENT ON COLUMN "account"."role" IS 'role people choose when signing up';
...
COMMENT ON COLUMN "recording"."yearly_club_championship_id" IS 'nullable because the club competition may not be a part of a yearly club championship';
...
COMMENT ON TABLE "equivalent_round" IS 'condition for 2 rounds to be considered equivalent: same category id
';

COMMENT ON COLUMN "range"."target_face_id" IS 'type of target face used';

COMMENT ON TABLE "request_competition_form" IS 'General rule is that archers and recorders can only choose to fill one field among 2 fields (yearly_club_championship_id and club_competition_id), they cannot fill 2 fields together. 
Only archers need to fill out the round_id field no matter what, recorders cannot fill the round_id field
';

COMMENT ON COLUMN "request_competition_form"."yearly_club_championship_id" IS 'nullable if the club competition has no yearly club championship';
...
COMMENT ON COLUMN "request_competition_form"."round_id" IS 'null if person is recorder';
...
COMMENT ON COLUMN "round_schedule"."datetime_to_end" IS 'when a round is expected to end';
...
COMMENT ON TABLE "friendship_link" IS '-- we will use the rule to make sure not to store redundant friendship link. For ex: we do not want to add a row (account_one_id = 2, account_two_id = 1) while there has already been an existing (account_one_id = 1, account_two_id_id = 2) row
...
COMMENT ON TABLE "friendship_request_form" IS 'we do not have form id is that we assume only one friend request being sent to avoid spam, accounts can send another friend request if record with (sender_id, reciever_id) has been deleted. Deletion only happens if unfriending happens';

COMMENT ON TABLE "group_request_form" IS 'we do not have form id is that we assume only one group request being sent to avoid spam, accounts can send another group request if record with (sender_id, reciever_id) has been deleted. Deletion only happens if the sender is no longer in group';
...
ALTER TABLE "request_competition_form" ADD FOREIGN KEY ("yearly_club_championship_id") REFERENCES "yearly_club_championship" ("yearly_club_championship_id");

ALTER TABLE "recording" ADD FOREIGN KEY ("yearly_club_championship_id") REFERENCES "yearly_club_championship" ("yearly_club_championship_id");
